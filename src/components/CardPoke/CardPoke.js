...
import { Container, CircleBlue, CircleRed, CircleYellow, CircleGreen, ContainerCircles, BorderBlack, BorderWhite, ImgPoke, Icon, ButtonPoke, ContainerButton } from './style'
    ...
    const getImgPoke = () => {
        ...
        axios.get(props.poke.url)
            .then((res) => {
                ...
                setImgPoke(res.data.sprites.versions["generation-v"]["black-white"].animated.front_default)
            })
            .catch((err) => {
                console.log(err)
            })
